...
BLECharacteristic adcCharacteristic("20a4a273-c214-4c18-b433-329f30ef7275", BLERead | BLENotify, 16);
...
void updateBLE(float reading0, float reading1) {
  data[i++] = reading0;
  data[i++] = reading1;
  if (i >= 4) {
...
void readExternalADC() {
  Ad7124_Readings readings[2];
  adc.readVolts(readings, 2);
  float packet[2];
  packet[0] = (float)readings[0].value;
  packet[1] = (float)readings[1].value;
  Serial.write((byte*)packet, sizeof(packet));
  Serial.write('\n');
  updateBLE(readings[0].value, readings[1].value);
}
...
void setup() {
  ...
  // second channel for vertical movements
  adc.setup[1].setConfig(AD7124_Ref_Internal, AD7124_Gain_1, true);
  adc.setup[1].setFilter(AD7124_Filter_SINC4, samplesPerSecondVal, AD7124_PostFilter_NoPost, false);
  adc.setChannel(1, 1, AD7124_Input_AIN2, AD7124_Input_AIN0, true);
